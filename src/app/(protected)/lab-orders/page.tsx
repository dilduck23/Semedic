...
import { LabTestCard } from "@/components/lab-orders/lab-test-card";
import { LabTestCategories } from "@/components/lab-orders/lab-test-categories";
import { useLabTestTypes, useLabOrders } from "@/hooks/use-lab-orders";
...
export default function LabOrdersPage() {
  ...
  const { data: testTypes, isLoading: loadingTypes } = useLabTestTypes(category);
  const { data: activeOrders } = useLabOrders("active");
  const { data: completedOrders } = useLabOrders("completed");
  const { setTestType } = useLabOrderStore();

  const handleSelectTest = (test: LabTestType) => {
    setTestType(test.id, test.name, test.price);
    router.push(`${ROUTES.LAB_ORDERS}/book`);
  };
    ...
    <>
      <header className="pt-12 lg:pt-6 pb-4 px-6 bg-card sticky top-0 z-20 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold">Laboratorio a Domicilio</h1>
        </div>
      </header>
      ...
      <main className="px-6 py-6 max-w-md mx-auto lg:max-w-3xl pb-28">
        <Tabs defaultValue="catalog" className="w-full">
          ...
          <TabsContent value="catalog" className="space-y-4">
            <LabTestCategories selected={category} onSelect={setCategory} />
          ...
          <TabsContent value="active" className="space-y-3">
            {!activeOrders || activeOrders.length === 0 ? (
              <div className="text-center pt-8">
                <MaterialIcon name="science" className="text-gray-300 text-5xl mb-3" />

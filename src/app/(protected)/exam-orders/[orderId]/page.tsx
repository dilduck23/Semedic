...
import { ResultsCard } from "@/components/exam-orders/results-card";
import { useExamOrder, useCancelExamOrder } from "@/hooks/use-exam-orders";
...
export default function ExamOrderDetailPage({
  ...
  const { data: order, isLoading } = useExamOrder(orderId);
  const cancelOrder = useCancelExamOrder();
  ...
  return (
    <>
      ...
      <main className="px-6 py-6 max-w-md mx-auto lg:max-w-lg pb-28 space-y-6">
          ...
          <CardContent className="pt-6 space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center">
                <MaterialIcon
                  name={order.exam_type?.icon_name || "biotech"}
                  className="text-orange-600 dark:text-orange-400"
                />
              </div>
              <div>
                <h2 className="font-bold">{order.exam_type?.name}</h2>
                <p className="text-xs text-muted-foreground">
                  {order.exam_type?.category}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2">
                <MaterialIcon name="local_hospital" className="text-muted-foreground text-base" />
                <span>{order.medical_center?.name}</span>
              </div>
              <div className="flex items-center gap-2">
                <MaterialIcon name="location_on" className="text-muted-foreground text-base" />
                <span>{order.medical_center?.address}</span>
              </div>
              <div className="flex items-center gap-2">
        ...
        {order.exam_type?.preparation_instructions && (
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <OrderTimeline currentStatus={order.status} type="exam" />
          </CardContent>
        </Card>

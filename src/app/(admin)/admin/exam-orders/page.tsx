...
import { useAdminExamOrders, useUpdateExamOrderStatus } from "@/hooks/use-admin";
import { ORDER_STATUS_LABELS, ORDER_STATUS_COLORS } from "@/lib/constants";
import type { ExamOrder, OrderStatus, Profile } from "@/types";
import { toast } from "sonner";

type ExamOrderRow = ExamOrder & { patient?: Profile };

export default function AdminExamOrdersPage() {
  const { data: orders = [], isLoading } = useAdminExamOrders();
  const updateStatus = useUpdateExamOrderStatus();
  ...
  const columns: ColumnDef<ExamOrderRow>[] = [
    {
      accessorKey: "patient",
      header: "Paciente",
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: "Examen",
      cell: ({ row }) => {
        const type = row.original.exam_type as unknown as { name: string; category: string } | undefined;
        return (
          <div>
    ...
    },
    {
      ...
      header: "Centro",
      cell: ({ row }) =>
        (row.original.medical_center as unknown as { name: string })?.name || "—",
    },
    {
  ...
  return (
    <AdminPageShell title="Ordenes de Examenes" description="Gestion de ordenes de examenes medicos">
      <DataTable
        columns={columns}
        data={filteredOrders}
        isLoading={isLoading}
